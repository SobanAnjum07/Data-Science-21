...
def establish_connection():
    return mysql.connector.connect(
        host="localhost", user="root", password="#@#$%$@", database="facebook" #i have not shown the password here because of security reasons
    )
...
def generate_members(total_members):
    ...
    for i in range(total_members):
        ...
        email = (
            f"{full_name.replace(' ', '').lower()}{i}@{random.choice(email_domains)}"
        )
        members.append((full_name, email))

    db_cursor.executemany(
        "INSERT INTO Members (full_name, contact_email) VALUES (%s, %s)", members
    )
    db_conn.commit()
    db_conn.close()
...
def generate_articles(total_articles):
    ...
    db_cursor.executemany(
        "INSERT INTO Articles (author_id, body, created_at) VALUES (%s, %s, %s)",
        articles,
    )
    db_conn.commit()
    db_conn.close()
...
def generate_feedbacks(total_feedbacks):
    ...
    for _ in range(total_feedbacks):
        ...
        commented_at = datetime.datetime.now() - datetime.timedelta(
            days=random.randint(0, 365)
        )
        feedbacks.append((article_id, member_id, feedback_text, commented_at))

    db_cursor.executemany(
        "INSERT INTO Feedback (article_id, commenter_id, comment, commented_at) VALUES (%s, %s, %s, %s)",
        feedbacks,
    )
    db_conn.commit()
    db_conn.close()
...
def generate_reactions(total_reactions):
    ...
    for _ in range(total_reactions):
        article_id = random.choice(article_ids)
        member_id = random.choice(member_ids)
        reacted_at = datetime.datetime.now() - datetime.timedelta(
            days=random.randint(0, 365)
        )
        reactions.append((article_id, member_id, reacted_at))

    db_cursor.executemany(
        "INSERT INTO Reactions (article_id, reactor_id, reacted_at) VALUES (%s, %s, %s)",
        reactions,
    )
    db_conn.commit()
    db_conn.close()
